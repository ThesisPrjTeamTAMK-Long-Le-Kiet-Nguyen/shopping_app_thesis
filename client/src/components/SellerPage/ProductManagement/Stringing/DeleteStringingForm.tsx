...
const deleteFormSchema = z.object({
  id: z.string().min(2, {
    message: "Stringing ID is required.",
  }),
...
export default function DeleteStringingForm() {
  ...
  const handleConfirm = async () => {
    try {
      ...
      switch (deleteType) {
        case 'full':
          response = await deleteProduct('stringings', formData.id)
          break
        case 'color':
          if (!formData.colorId) {
          ...
          response = await deleteProductColor('stringings', formData.id, formData.colorId)
          break
      }

      if (response.success) {
        toast.success(`Stringing ${deleteType} deleted successfully`)
        setDialogOpen(false)
        form.reset()
      } else {
        toast.error(`Failed to delete stringing ${deleteType}`)
      }
    } catch (error) {
      toast.error(`Error deleting stringing ${deleteType}`)
    }
  }
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div>
                <span>Are you sure you want to delete this {deleteType === 'full' ? 'stringing' : 'color'}?
                  This action cannot be undone.</span>

                {formData && (
                  <div className="mt-2 space-y-2 bg-gray-50 p-3 rounded-md">
                    <div className="grid grid-cols-1 gap-2">
                      <div><span className="font-medium">Stringing ID:</span> {formData.id}</div>
                      {formData.colorId && (
