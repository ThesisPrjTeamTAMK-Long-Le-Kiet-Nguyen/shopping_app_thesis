...
const GripList = () => {
  ...
  const fetchGripList = useCallback(async () => {
    try {
      setIsLoading(true);
      setError(null);
  ...
  // Memoize grip card component
  const GripCard = useMemo(() => {
    ...
    return (
      <div className="p-6">
        <div className="text-center space-y-4">
          ...
          <Button onClick={fetchGripList}>Try Again</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {grips.map((grip) => (
          <GripCard key={grip.id} grip={grip} />
        ))}
      </div>
    </div>
  );
