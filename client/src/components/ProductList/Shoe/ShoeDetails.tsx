...
import { Shoe, ShoeColor, ShoeSize, CartItem } from '../../../types'
...
const ShoeDetails = () => {
  ...
  const handleColorChange = useCallback((color: ShoeColor) => {
  ...
  const handleSizeChange = useCallback((size: ShoeSize) => {
    setSelectedSize(size);
  }, []);

  // Type guard function
  const hasTypes = useCallback((color: ShoeColor): color is ShoeColor & { types: ShoeSize[] } => {
    return Array.isArray(color.types) && color.types.length > 0;
  }, []);

  // Stock check function
  const hasStock = useCallback((size: ShoeSize): boolean => {
  ...
  }, [shoe, selectedColor, selectedSize, hasStock]);

  // Memoize color buttons
  const colorButtons = useMemo(() => {
    if (!shoe) return [];
    return shoe.colors.map((colorInfo, index) => (
      <Button
        key={index}
  ...
  }, [shoe, selectedColor, handleColorChange, hasTypes]);

  // Memoize size buttons
  const sizeButtons = useMemo(() => {
    if (!selectedColor || !hasTypes(selectedColor)) return [];
    return selectedColor.types.map((sizeInfo, index) => (
      <Button
        key={index}
  ...
  }, [selectedColor, selectedSize, handleSizeChange, hasTypes, hasStock]);
    ...
    return (
      <div className="container mx-auto px-4 py-6">
        <div className="text-center space-y-4">
          ...
          <Button onClick={fetchShoeDetails}>Try Again</Button>
        </div>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
              ...
              <Button
                ...
                disabled={!selectedColor || !selectedSize || !hasStock(selectedSize) || isAddingToCart}
              >
                <ShoppingCart className="mr-2 h-5 w-5" />
                {isAddingToCart ? 'Adding to Cart...' : 
                 !selectedColor || !selectedSize || !hasStock(selectedSize) ? 'Out of Stock' : 
